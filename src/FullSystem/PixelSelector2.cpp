...
PixelSelector::PixelSelector(int w, int h)
{
	...
	// 计算直方图（每个格子大小为32*32），直方图的个数为(1+w/32)*(1+h/32)，加一是为了避免不能整除
...
/**
 * @brief 占据 below% 的梯度值作为阈值
 * 
 * @param hist 		直方图，hist[0]表示该格子所有像素的个数
 * @param below 	阈值百分比（一般为0.5）
 * @return int 		返回格子的阈值
 */
int computeHistQuantil(int* hist, float below)
{
	int th = hist[0]*below+0.5f;	// th = 所有像素的个数*below
	for(int i=0;i<90;i++)
	{
		th -= hist[i+1];			// 梯度值为0-i的所有像素个数占 below
		if(th<0) return i;			// 梯度阈值
	}
	return 90;
}

/**
 * @brief 生成梯度直方图, 为每个block计算阈值
 * 
 * @param fh 当前帧
 */
void PixelSelector::makeHists(const FrameHessian* const fh)
{
	...
	// 每个blocks大小为32*32, 不是论文里的32*32个网格
	int w32 = w/32;
	int h32 = h/32;
	thsStep = w32;
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			thsSmoothed[x+y*w32] = (sum/num) * (sum/num);	// 平方
...
/**
 * @brief 根据阈值选择不同层上符合要求的像素
 * 
 * @param fh 					当前帧
 * @param map_out 				选中的像素点及所在层
 * @param pot 					选点的范围大小, 一个pot内选一个
 * @param thFactor 				阈值因子(乘数)
 * @return Eigen::Vector3i 		返回的是第0、1、2层选择的点的个数
 */
Eigen::Vector3i PixelSelector::select(const FrameHessian* const fh,
		float* map_out, int pot, float thFactor)
{

	// const在 * 的左边表示指针内容不可改, const在 * 的右边表示指针不可改
	// 等价 const Eigen::Vector3f * const
	...
	// 金字塔层阈值的减小倍数（作者设置为0.75）
	float dw1 = setting_gradDownweightPerLevel;	// 第二层
	float dw2 = dw1*dw1;						// 第三层

	// 第2层1个pot对应第1层4个pot, 第1层1个pot对应第0层的4个pot
	// 第0层的4个pot里面只要选一个像素, 就不在对应高层的pot里面选了，但是还会在第0层的每个pot里面选大于阈值的像素
	// 阈值随着层数增加而下降
	// 从顶层向下层遍历, 写的挺有意思!


	int n3=0, n2=0, n4=0;
	// 第2层中, 每隔pot选一个点遍历（pot大小作者设置为3）
